...
class Receive extends MY_Controller {
    ...
    public function create()
    {
        ...
        if (
            ...
            echo json_encode([
                'status'  => false,
                'message' => 'Plant, tanggal receive, dan supplier wajib diisi'
            ]);
            return;
        }
        ...
        $plant = $post['PLANT'];
        $po = trim($post['PO'] ?? '');
        $po = ($po === '' || strtolower($po) === 'null') ? null : $po;

        /* ================= VALIDASI PLANT USER ================= */
        if (!$this->Receive_model->user_has_plant($username, $plant)) {
            echo json_encode([
                'status'  => false,
                'message' => 'Plant tidak diizinkan untuk user ini'
            ]);
            return;
        }

        /* ================= VALIDASI PO vs PLANT ================= */
        if ($po !== null) {
        ...
        if ($detailRaw === null) {
            echo json_encode([
                'status'  => false,
                'message' => 'DETAIL tidak terkirim ke server'
            ]);
            return;
        }
        ...
        if (empty($detailArr) || !is_array($detailArr)) {
            echo json_encode([
                'status'  => false,
                'message' => 'Detail receive wajib diisi'
            ]);
            return;
        }
        ...
        foreach ($detailArr as $row) {
            ...
            $rows[] = [
                ...
                'SEQ_NO'     => $seq,
                'PO'         => $po,
                'PO_SEQ'     => isset($row['PO_SEQ']) ? $row['PO_SEQ'] : null,
                'MATERIAL'   => $row['MATERIAL'],
                'JUMLAH'     => (float) $row['JUMLAH'],
                'BERAT'      => (float) $row['BERAT'],
                'HARGA'      => $this->normalize_number($row['HARGA']),
                'TOTAL'      => $this->normalize_number($row['TOTAL']),
                'STATUS'     => 'RECEIVED',
                'CREATED_AT' => date('Y-m-d H:i:s'),
                'CREATED_BY' => $username
            ];
        ...
        if (empty($rows)) {
            ...
            echo json_encode([
                'status'  => false,
                'message' => 'Detail receive tidak valid'
            ]);
            return;
        }
        ...
        if ($po !== null) {
            $this->Receive_model->update_po_status($po, $plant, 'RECEIVED');
        }
        ...
        echo json_encode([
            ...
            'message' => $this->db->trans_status()
                ? 'Receive berhasil disimpan'
                : 'Gagal menyimpan Receive'
        ]);
        exit;
    }
