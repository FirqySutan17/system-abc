...
class StockActual_model extends CI_Model {
    ...
    /* =========================================================
       STOCK & ITEM (TIDAK DIUBAH – SUDAH BENAR)
    ========================================================= */
    ...
    public function get_stock_by_item($item, $plant)
    {
        ...
        return [
            ...
            'stock_bw'  => (float)($row['stock_bw']  ?? 0)
        ];
    }

    /* =========================================================
       HEADER & DETAIL (TIDAK DIUBAH)
    ========================================================= */
    ...
    {
        $date = date('Ymd', strtotime($sa_date));
        $prefix = $date . 'SA';

        $this->db->where('PLANT', $plant);
        $this->db->like('STOCK_ACTUAL', $prefix, 'after');
        $this->db->order_by('STOCK_ACTUAL', 'DESC');
        $row = $this->db->get('mst_stock_actual')->row();

        $seq = $row ? (int)substr($row->STOCK_ACTUAL, -4) + 1 : 1;
